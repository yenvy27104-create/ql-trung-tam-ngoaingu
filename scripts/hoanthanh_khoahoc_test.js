/**
 * GIẢ LẬP MỞ KHÓA HOÀN THÀNH KHÓA HỌC & ĐĂNG KÝ GIỮ CHỐI 3 KHÓA
 * 
 * Cách chạy:
 * 1. Đăng ký 3 khóa cùng lúc (A Đang học, B & Cấp tốc Giữ chỗ):
 *    node scripts/hoanthanh_khoahoc_test.js setup
 * 
 * 2. Nhập điểm Cuối kỳ đạt -> Hoàn thành Khóa A -> Tự động mở khóa B & Cấp Tốc:
 *    node scripts/hoanthanh_khoahoc_test.js completeA
 * 
 * 3. Dọn dẹp reset:
 *    node scripts/hoanthanh_khoahoc_test.js reset
 */

const pool = require('../models/db');

async function runCourseProgressionTest() {
  const arg = (process.argv[2] || 'setup').toLowerCase();

  console.log('========================================================================');
  console.log('🧪 GIẢ LẬP HOÀN THÀNH KHÓA HỌC A & MỞ KHÓA B KHÓA GIỮ CHỖ CÙNG LÚC');
  console.log('========================================================================\n');
  ...
  try {
    ...
    console.log(`👤 HỌC VIÊN DEMO: ${student.HoTen} (Mã HV: ${hId})\n`);
    ...
    if (arg === 'setup') {
      ...
      // Khóa B: Đang học (Chuyển 🔒 Giữ chỗ chờ khóa A)
      await pool.query(
      ...
      // Khóa Cấp tốc / Luyện đề: Đang học (Chuyển 🔒 Giữ chỗ chờ khóa A)
      await pool.query(
      ...
      console.log('\n👉 Bạn tải lại trang Học viên (F5), Khóa B và C sẽ hiển thị nhãn "🔒 Giữ chỗ"!');
      console.log('👉 Khi muốn demo Hoàn thành Khóa A để tự động MỞ KHÓA B & C, chạy tiếp lệnh:');
      console.log('   node scripts/hoanthanh_khoahoc_test.js completeA\n');
      ...
      await pool.query("DELETE FROM dangkyhoc WHERE MaHocVien = ? AND MaLopHoc IN (?, ?, ?)", [hId, idA, idB, idCT]);
      await pool.query("DELETE FROM quanlydiem WHERE MaHocVien = ? AND MaLopHoc = ?", [hId, idA]);
      console.log('🧹 ĐÃ DỌN DẸP SẠCH BẢN GHI DEMO HOÀN THÀNH KHÓA HỌC.');
    }
